...
Public Class frm_delinquency
    ...
    Private Sub generate_delinquency()
        'REPORT OBJECT
        Dim MyRpt As New delinquency

        'DATASET, AND DATAROW OBJECTS NEEDED TO MAKE THE DATA SOURCE
        Dim row As DataRow = Nothing
        Dim DS As New DataSet

        'use to filter officer
        Dim cOfficer As String
        Dim cCollCode As String
        Dim cCtrcode As String
        Dim cmultiproduct As String
        ...
        If multiproduct = "N" Then
            ...
            'mao ni ang original, ato sa e remarks
            'sql += "select x.*,((x.prnloan)-x.TotalPrnPaid)as Loanbal,((x.loanamnt+x.interestdue)-(x.TotalPrnPaid+x.TotalintPaid))as runbal,(x.totalprndue-x.TotalPrnPaid) as PrnPar,(x.totalintdue-x.TotalIntPaid) as IntPar,case when ((x.totalintdue-x.TotalIntPaid) >1) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else (case when ((x.totalprndue-x.TotalPrnPaid)>1) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else 0 end)  end as PARDays from ("

            'mao ni ang modified - 10-09-2023, dapat greater than zero, dili sya greater than 1
            sql += "select x.*,((x.prnloan)-x.TotalPrnPaid)as Loanbal,((x.loanamnt+x.interestdue)-(x.TotalPrnPaid+x.TotalintPaid))as runbal,(x.totalprndue-x.TotalPrnPaid) as PrnPar,(x.totalintdue-x.TotalIntPaid) as IntPar,case when ((x.totalintdue-x.TotalIntPaid) >1) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else (case when (  (x.totalprndue-x.TotalPrnPaid) + (x.totalprndue-x.TotalPrnPaid) > 0 ) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else 0 end)  end as PARDays from ("





            'sql += "select x.*,((x.prnloan)-x.TotalPrnPaid)as Loanbal,((x.loanamnt+x.interestdue)-(x.TotalPrnPaid+x.TotalintPaid))as runbal,(x.totalprndue-x.TotalPrnPaid) as PrnPar,(x.totalintdue-x.TotalIntPaid) as IntPar,case when ((x.totalintdue-x.TotalIntPaid) > 0 ) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else (case when ((x.totalprndue-x.TotalPrnPaid) > 0 ) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else 0 end)  end as PARDays from ("
        ...
        Else
            ...
            'mao ni ang modified - 10-09-2023, dapat greater than zero, dili sya greater than 1
            sql += "select x.*,((x.prnloan)-x.TotalPrnPaid)as Loanbal,((x.loanamnt)-(x.TotalPrnPaid+x.TotalintPaid))as runbal,(x.totalprndue-x.TotalPrnPaid) as PrnPar,(x.totalintdue-x.TotalIntPaid) as IntPar,case when ( (x.totalprndue-x.TotalPrnPaid) + (x.totalintdue-x.TotalIntPaid) > 0 ) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else (case when ((x.totalprndue-x.TotalPrnPaid) > 0 ) then datediff(day,x.Duedate,'" + gendate.Value.AddDays(1) + "') else 0 end)  end as PARDays from ("
        ...
        If cboption.Text = "By Officer" Then
            If txtcenter.SelectedValue <> "All" Then
                sql += " and a.CollCode='" & txtcenter.SelectedValue.ToString & "'"

                'collection code
                cCollCode = txtcenter.SelectedValue.ToString
            End If
        Else
            If txtcenter.SelectedValue <> "All" Then
                sql += " and a.ctrcode='" + txtcenter.SelectedValue + "'"
                cCtrcode = txtcenter.SelectedValue
            End If
        End If
        ...
        'STORED PROCEDURE
        conn()
        Dim cmdDeliquency As New SqlCommand

        cmdDeliquency.Connection = myConn
        cmdDeliquency.CommandTimeout = 300
        cmdDeliquency.CommandType = CommandType.StoredProcedure
        cmdDeliquency.CommandText = "DelinquencyReport"
        cmdDeliquency.Parameters.AddWithValue("@gendate", gendate.Value)
        cmdDeliquency.Parameters.AddWithValue("@GL_loans", GL_loans)
        cmdDeliquency.Parameters.AddWithValue("@CollCode", cCollCode)
        cmdDeliquency.Parameters.AddWithValue("@ctrcode", cCtrcode)
        cmdDeliquency.Parameters.AddWithValue("@multiproduct", cmultiproduct)


        myConn.Open()
        rd = cmdDeliquency.ExecuteReader()
