...
Public Class frm_profile_by_occupation

    Private Sub generate_profile_by_occupation()
        'REPORT OBJECT
        Dim MyRpt As New loanprofileby_income
        ...
        conn()
        sql = "select distinct z.* from(select distinct w.occupcode,w.occupdesc,"
        'sql += "noofborrowers=isnull((select count(pnnumber) from loans where gl_loans='" + GL_loans + "' and memcode in(select memcode from members where age=w.age) and status='A'),0),"
        ...
        sql += " from Loans a,members b WHERE a.MemCode=b.MemCode and a.memcode in(select memcode from members where occupcode=w.occupcode) AND a.released='Y' and a.status<>'X' and a.releasedate<='" + dtfrom.Value + "' and a.gl_loans='" + GL_loans + "'"
        ...
        sql += " from Loans a,Members b where  a.status<>'X' and a.memcode in(select memcode from members where occupcode=w.occupcode) and a.MemCode=b.MemCode and a.released='Y' AND a.GL_loans='" + GL_loans + "') x"
        ...
        sql += " from Loans a,Members b where  a.status<>'X' and a.memcode in(select memcode from members where occupcode=w.occupcode) and a.MemCode=b.MemCode and a.released='Y' AND a.GL_loans='" + GL_loans + "') x"
        sql += ") y where y.PARdays>0),0)"
        sql += "from Occupation w)z order by z.occupdesc"
    ...
    Private Sub bttngen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttngen.Click
        ...
        thread.Start()
        generate_profile_by_occupation()
        SplashScreen.Close()
    End Sub
