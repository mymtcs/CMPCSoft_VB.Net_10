...
Public Class frm_profile_by_age

    Private Sub generate_ctr_incntv()
        'REPORT OBJECT
        Dim MyRpt As New loanprofileby_age
        ...
        conn()
        sql = "select distinct z.* from(select distinct datediff(year,w.birthdate,getdate()) as age,"
        ...
        sql += " from Loans a,members b WHERE a.MemCode=b.MemCode and a.memcode in(select memcode from members where age=w.age) AND a.released='Y' and a.status<>'X' and a.releasedate<='" + dtfrom.Value + "' and a.gl_loans='" + GL_loans + "'"
        ...
        sql += " from Loans a,Members b where  a.status<>'X' and a.memcode in(select memcode from members where age=w.age) and a.MemCode=b.MemCode and a.released='Y' AND a.GL_loans='" + GL_loans + "') x"
        ...
        sql += " from Loans a,Members b where  a.status<>'X' and a.memcode in(select memcode from members where age=w.age) and a.MemCode=b.MemCode and a.released='Y' AND a.GL_loans='" + GL_loans + "') x"
        sql += ") y where y.PARdays>0),0)"
        sql += "from members w)z order by z.age"
