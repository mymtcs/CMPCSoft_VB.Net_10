...
Public Class frm_CBUdeposit
    ...
    Private Sub frm_CBUwithdrawals_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Text = "CBU Deposit (" & frm_cbuledger.dtgridsaving.CurrentRow.Cells(0).Value & ")"
    ...
    Private Sub insert_to_disburse()
        conn()
        sql = "INSERT INTO CashiersTransaction(Payee,Debit,Credit,Reference,Trndate,Ttime,UserID,GLaccount,Remarks,AcctReference) VALUES ('" + frm_cbuledger.dtgridsaving.CurrentRow.Cells(1).Value + "'," + Double.Parse(txtamnt.Text) + ",0,'" + txtref.Text + "','" + txtpdate.Value + "','" + time.ToString + "','" + user.ToString + "','" + txtgl.SelectedValue + "','Deposit','0')"
        cmd = New SqlCommand(sql, myConn)
        myConn.Open()
        cmd.ExecuteNonQuery()
        myConn.Close()
    ...
    Private Sub RadButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadButton1.Click
        Dim bal As Decimal = Double.Parse(frm_cbuledger.dtgridsaving.CurrentRow.Cells(2).Value) + Double.Parse(txtamnt.Text)
        'If bal > 3000 Then
        '    MsgBox("CBU balance cannot be greater than the limited CBU subscription.", MsgBoxStyle.Exclamation, "Invalid")
        '    txtamnt.Focus()
        ...
        Else
            If MessageBox.Show("Click Yes to continue.", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = DialogResult.Yes Then
                conn()
                sql = "SELECT isnull((MAX(postno)),0) As postno FROM CBULedger WHERE CBUaccount='" + frm_cbuledger.dtgridsaving.CurrentRow.Cells(0).Value + "'"
                cmd = New SqlCommand(sql, myConn)
                myConn.Open()
                rd = cmd.ExecuteReader
                If rd.Read Then
