...
Public Class frm_loanProcess_indv
    ...
    Private Sub view_sched()
        ...
        lstgensched.Items.Clear()
        conn()
        sql = "SELECT LnNum,CONVERT(VARCHAR(12),DueDate,107),Principal,Interest,savings,CF,LH,AmntRcvbl FROM Loansched WHERE pnnumber='" + txtlno.Text + "' order by LnNum"
        cmd = New SqlCommand(sql, myConn)
        myConn.Open()
        rd = cmd.ExecuteReader()
        While (rd.Read())
    ...
    Private Sub gen_loantype()
        ...
        While (rd.Read())
            table1.Rows.Add(rd("loancode"), rd("loandesc") & " " & rd("Rate") & "%", rd("CF").ToString, rd("savingsprcnt").ToString)
        End While
        rd.Close()
        myConn.Close()
    ...
    Private Sub gen_interest()
        conn()
        sql = "SELECT Decimal_rate from Loantype where loancode ='" + txtloantype.SelectedValue + "'"
        cmd = New SqlCommand(sql, myConn)
        myConn.Open()
        rd = cmd.ExecuteReader
        If rd.Read Then
            loanrate = rd("Decimal_rate")
        End If
        rd.Close()
    End Sub
    ...
    Private Sub gen_loansched()
        ...
        If cboterms.Text = "Daily" Then
            loanrate = (loanrate / 30) * payable
        ElseIf cboterms.Text = "Week(s)" Then
            loanrate = (loanrate / 4) * payable
        ElseIf cboterms.Text = "Semi-Monthly" Then
            loanrate = (loanrate / 2) * payable
        ElseIf cboterms.Text = "Month(s)" Then
            loanrate = (loanrate / 1) * payable
        End If
        txtsch_weeks.Text = cboterms.Text

        txtschdinterestperann.Text = loanrate * 100
        'Try
        lstgensched.Items.Clear()
        ...
        If Me.Text = "Re-Construct Loan" Then
            ...
            intdue = Double.Parse(frm_recons_amount.ttlint) / payable
            savings = 0
            cf = 0
        Else
        ...
        For x As Integer = 0 To payable - 1
            ...
            If x = (payable - 1) Then
                ...
                If Me.Text = "Re-Construct Loan" Then
                    ...
                    lstintdue = Double.Parse(frm_recons_amount.ttlint) - Double.Parse(lblintdue.Text)
                Else
                    lstprndue = Double.Parse(txtlnamnt.Text) - Double.Parse(lblprndue.Text)
                    lstintdue = (Double.Parse(txtlnamnt.Text) * loanrate) - Double.Parse(lblintdue.Text)
                End If
        ...
        'For days As Integer = 0 To lstgensched.Items.Count - 1
        '    dtsched = lstgensched.Items(days).SubItems(1).Text
        '    'MsgBox(duedays.Day)
        '    If dtsched.DayOfWeek = DayOfWeek.Saturday Then
        '        lstgensched.Items(days).SubItems(1).Text = dtsched.AddDays(-1).ToString("D")
        '    ElseIf dtsched.DayOfWeek = DayOfWeek.Sunday Then
        '        lstgensched.Items(days).SubItems(1).Text = dtsched.AddDays(1).ToString("D")
        '    End If
        'Next
    ...
    Private Sub insert_sched()
        ...
        For i As Integer = 0 To lstgensched.Items.Count - 1
            ...
            If lstgensched.Items(i).SubItems(0).Text <> "" Then
                sql = "INSERT INTO Loansched(PnNumber,LnNum,DueDate,Principal,Interest,savings,CF,lh,AmntRcvbl,rngprin,rngint) VALUES "
                sql += "('" + txtlno.Text + "',"
    ...
    Private Sub bttnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttnsave.Click
        'Try
        ' count number of days from date release and 1st schedule of payment  
        Dim noofdays As Long = DateDiff("d", Date.Parse(dtrel.Text), Date.Parse(dtschedoffirst.Text))
        '
        'MsgBox(noofdays, MsgBoxStyle.Exclamation, "No. of Days")
        ...
        ElseIf txtloantype.SelectedValue = "" Then
            ...
            txtloantype.Focus()
        ElseIf txtsaacct.Text = "" And isP3 = "N" And Me.Text <> "Re-Construct Loan" Then
            MsgBox("Invalid selection of savings account.", MsgBoxStyle.Exclamation, "Invalid")
            txtsaacct.Focus()
        ElseIf multiproduct = "N" And txtsaacct.Text.Trim = "" And isP3 = "N" And Me.Text <> "Re-Construct Loan" Then
        ...
        ElseIf Double.Parse(txtlnamnt.Text) < 1 Then
            ...
            txtlnamnt.Focus()
        ElseIf noofdays > 30 Then ' check if date 1st payment must not be more than 30 days
            MsgBox("1st Schedule of payment must not be greater than 30 days.", MsgBoxStyle.Exclamation, "Invalid")
            dtschedoffirst.Focus()
        Else
            gen_cbuacct()
            gen_loan_rate()
            If Me.Text = "Edit Loan" Then
                If MessageBox.Show("Are you sure you want to update this loan?", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                    ...
                    sql += "isCheckedLH='" + chklh.Checked.ToString + "'"
            ...
            ElseIf Me.Text = "Re-Construct Loan" Then
                gen_pnnumber()
                loanrate = Double.Parse(frm_recons_amount.txtpdi.Text) + Double.Parse(frm_recons_amount.txtpdp.Text)
                conn()
                sql = "INSERT INTO Loans(Memcode,Pnnumber,CollCode,FundCode,LoanAmnt,TotalDeduction,netproceeds,IntRate,modeofpayment,LoanDate,Status,Released,userid,loantype,cycle,firstpaymentdate,releasedate,col_type,Purpose,PayNo,PenaltyRate,Accountnumber,GL_loans,isCheckedLH,Restructured) VALUES "
                sql += "('" + txtmember.SelectedValue + "',"
                sql += "'" + txtlno.Text + "',"
                ...
                sql += "'Restructured Loan',"
                sql += "" + payable.ToString + ","
                sql += "" + penaltyrate.ToString + ","
                sql += "'" + txtsaacct.Text + "',"
                sql += "'" + GL_loans + "',"
                sql += "'" + chklh.Checked.ToString + "',"
                sql += "'Yes'"
                sql += ")"
                ' MsgBox(sql.ToString)
                cmd = New SqlCommand(sql, myConn)
            ...
            ElseIf Me.Text = "View Loan" Then
                ...
                Else
                    conn()
                    sql = "UPDATE Loans SET CollCode='" + txtcoll.SelectedValue + "',"
                    sql += "userid='" + user.ToString + "',ischeckedLH='" + chklh.Checked.ToString + "'"
                    sql += " WHERE pnnumber='" + txtlno.Text + "'"
                    cmd = New SqlCommand(sql, myConn)
                    myConn.Open()
                    cmd.ExecuteNonQuery()
                    ...
                    'conn()
                    'sql = "UPDATE loansched set LH='" + lh.ToString + "',accountnumber='" + txtsaacct.Text + "' WHERE pnnumber='" + txtlno.Text + "'"
                    'cmd = New SqlCommand(sql, myConn)
                    'myConn.Open()
                    'cmd.ExecuteNonQuery()
                    'myConn.Close()
            ...
            Else
                ...
                If Not rd.Read Then
                    If MessageBox.Show("Are you sure you want to save this loan?", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                        ...
                        conn()
                        sql = "INSERT INTO Loans(Memcode,Pnnumber,CollCode,FundCode,LoanAmnt,TotalDeduction,netproceeds,IntRate,modeofpayment,LoanDate,Status,Released,userid,loantype,cycle,firstpaymentdate,releasedate,col_type,Purpose,PayNo,PenaltyRate,Accountnumber,GL_loans,isCheckedLH) VALUES "
                        sql += "('" + txtmember.SelectedValue + "',"
                        sql += "'" + txtlno.Text + "',"
                        sql += "'" + txtcoll.SelectedValue.ToString + "',"
                        sql += "'" + txtfunding.SelectedValue.ToString + "',"
            ...
            If rd.Read Then
                ...
                Else
                    ...
                    Else
                        If rd("base_on_month") = "Y" Then
                            If cboterms.Text = "Week(s)" Then
                                Dim fixedamnt As Double = Double.Parse(rd("fixedamnt")) / 4
                                dtgrd_deduction.CurrentRow.Cells(2).Value = fixedamnt * Double.Parse(txtpayno.Value)
                            ElseIf cboterms.Text = "Semi-Monthly" Then
                                Dim fixedamnt As Double = Double.Parse(rd("fixedamnt")) / 2
                                dtgrd_deduction.CurrentRow.Cells(2).Value = fixedamnt * Double.Parse(txtpayno.Value)
                            Else
                                Dim fixedamnt As Double = Double.Parse(rd("fixedamnt"))
                                dtgrd_deduction.CurrentRow.Cells(2).Value = fixedamnt * Double.Parse(txtpayno.Value)
                            End If
                        Else
    ...
    Private Sub gen_savbalance()
        Try
            conn()
            sql = "select balance=isnull((sum(debit-credit)),0) from accountledger where accountnumber='" + txtsaacct.Text + "'"
            cmd = New SqlCommand(sql, myConn)
            myConn.Open()
            rd = cmd.ExecuteReader
            If rd.Read Then
    ...
    Private Sub dtschedoffirst_ValueChanged(sender As System.Object, e As System.EventArgs) Handles dtschedoffirst.ValueChanged
    ...
    Private Sub chklh_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chklh.CheckedChanged
