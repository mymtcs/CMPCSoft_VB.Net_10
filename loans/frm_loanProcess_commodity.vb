...
Public Class frm_loanProcess_commodity
    ...
    Private Sub view_subproducts()
        ...
        table1.Rows.Add("--select--", "--select--", "--select--")
        While (rd.Read())
    ...
    Private Sub gen_members()
        ...
        Else
            ...
            'sql = "select x.* from(select a.memcode,a.fullname,"
            'sql += "ttlloans=isnull((select count(memcode) from loans where memcode=a.memcode and status='A' and gl_loans='1050-02'),0)"
            'sql += "from members a where status='A')x where x.ttlloans<3"
        End If
        cmd = New SqlCommand(sql, myConn)
        myConn.Open()
        rd = cmd.ExecuteReader
    ...
    Private Sub display_members_list()
        conn()
        'sql = "select memcode,fullname from members where memcode not in(select memcode from loans where status='A' and gl_loans='" + GL_loans + "') and status='A' and fullname like '%" & txtsearch.Text.Trim & "%' order by fullname"
        sql = "select top 200 memcode,fullname from members where status='A' and fullname like '%" & txtsearch.Text.Trim & "%' order by fullname"
        cmd = New SqlCommand(sql, myConn)
        myConn.Open()
        rd = cmd.ExecuteReader
    ...
    Public Sub creategview_items()
        ...
        If Me.Text = "Edit Loan" Or Me.Text = "View Loan" Then
            conn()
            sql = "SELECT * FROM LoanItems WHERE pnnumber='" + loannumber + "'"
            cmd = New SqlCommand(sql, myConn)
            myConn.Open()
            rd = cmd.ExecuteReader
            While rd.Read
    ...
    Private Sub gen_sched()
        If txtctr.Text.Trim <> "" Then
            ...
            If rd.Read Then
                ...
                Dim frstched As Date
                If cboterms.Text = "Week(s)" Then
                    frstched = today.AddDays(-dayDiff + 7)
                ElseIf cboterms.Text = "Semi-Monthly" Then
                    frstched = today.AddDays(-dayDiff + 15)
                ElseIf cboterms.Text = "Month(s)" Then
                    frstched = today.AddDays(-dayDiff + 30)
                End If
    ...
    Private Sub gen_loansched()
        ...
        If cboterms.Text = "Daily" Then
            If chkfixed.Checked = True Then
                subrate = subrate
            Else
                subrate = (subrate / 30) * payable
            End If
        ElseIf cboterms.Text = "Week(s)" Then
            If chkfixed.Checked = True Then
                subrate = subrate
            Else
                subrate = (subrate / 4) * payable
            End If
        ElseIf cboterms.Text = "Semi-Monthly" Then
            If chkfixed.Checked = True Then
                subrate = subrate
            Else
                subrate = (subrate / 2) * payable
            End If

        ElseIf cboterms.Text = "Month(s)" Then
            subrate = (subrate / 1) * payable
        End If
        txtschdlnamnt.Text = txtlnamnt.Text
        txtschdinterestperann.Text = subrate
        lstgensched.Items.Clear()
        Dim dtsched As Date = dtschedoffirst.Value
        Dim dtschedx As Date = dtschedoffirst.Value
        payno = 0
        intdue = 0
        prndue = 0
        ttlmarkup = 0
        ...
        If Me.Text = "Re-Construct Loan" Then
            prndue = Double.Parse(frm_recons_amount.txtprin.Text) / payable
            intdue = Double.Parse(frm_recons_amount.txtpdi.Text) / payable
            savings = 0
            cf = 0
        Else
            If dtgrid_items.Rows.Count = 0 Then
                ttlprn = Double.Parse(txtlnamnt.Text)
                prndue = ttlprn / payable
            Else
                prndue = ttlprn / payable
            End If

            If subrate > 0 Then
                ttlint = Double.Parse(txtlnamnt.Text) * subrate
            End If
            intdue = ttlint / payable
        End If
        ...
        For x As Integer = 0 To payable - 1
            ...
            If x = (payable - 1) Then
                ...
                Else
                    lstprndue = ttlprn - Double.Parse(lblprndue.Text)
                    lstintdue = ttlint - Double.Parse(lblintdue.Text)
                End If
    ...
    Private Sub insert_deductions()
        'conn()
        'For i As Integer = 0 To dtgrd_subproducts.Rows.Count - 1
        '    If dtgrd_subproducts.Rows(i).Cells(1).Value <> "" Then
        '        sql = "INSERT INTO LoansDeduction(PnNumber,code,Description,Amount,Acro) VALUES ('" + txtlno.Text + "','" + dtgrd_subproducts.Rows(i).Cells(1).Value + "','" + dtgrd_subproducts.Rows(i).Cells(3).Value + "'," + dtgrd_subproducts.Rows(i).Cells(2).Value.ToString + ",'" + dtgrd_subproducts.Rows(i).Cells(4).Value.ToString + "')"
        '        cmd = New SqlCommand(sql, myConn)
        '        myConn.Open()
        '        cmd.ExecuteNonQuery()
        '        myConn.Close()
    ...
    Private Sub bttnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bttnsave.Click
        'Try
        If txtcycle.Text.Trim = "" Or IsNumeric(txtcycle.Text) = False Then
            txtcycle.Text = 1
        End If
        ...
        ElseIf txtloantype.SelectedValue = "" Then
            ...
            txtloantype.Focus()
        ElseIf txtsubproduct.SelectedValue = "" Or txtsubproduct.SelectedValue = "--select--" Then
            MsgBox("Invalid selection of sub product.", MsgBoxStyle.Exclamation, "Invalid")
            txtsubproduct.Focus()
        ElseIf txtsaacct.SelectedValue = "" And multiproduct = "N" Then
        ...
        Else
            ...
            End If
            conn()
            sql = "SELECT * from loans where memcode='" + txtmember.SelectedValue + "' and gl_loans='" + GL_loans + "' and released='Y' and Subproductcode='" + txtsubproduct.SelectedValue + "' and status='A'"
            cmd = New SqlCommand(sql, myConn)
            myConn.Open()
            rd = cmd.ExecuteReader
            If rd.Read Then
                MsgBox("This member is already active in current subproduct.", MsgBoxStyle.Exclamation, "Message")
            Else
                gen_cbuacct()
                gen_loan_rate()
...
3:                  If usertype = "Admin" Then
                    ...
                    Else
                        ...
                        sql += "Firstpaymentdate='" + dtschedoffirst.Value + "',"
                        sql += " ctrcode='" + txtctr.SelectedValue.ToString + "',grpcode='" + cbogrpcode.Text + "' WHERE pnnumber='" + txtlno.Text + "'"
                        cmd = New SqlCommand(sql, myConn)
                        myConn.Open()
                        cmd.ExecuteNonQuery()
                        myConn.Close()
                ...
                ElseIf Me.Text = "Re-Construct Loan" Then
                    ...
                    conn()
                    sql = "INSERT INTO Loans(Memcode,Pnnumber,CollCode,FundCode,LoanAmnt,TotalDeduction,netproceeds,IntRate,modeofpayment,LoanDate,Status,Released,userid,loantype,cycle,firstpaymentdate,releasedate,ctrcode,grpcode,col_type,Purpose,PayNo,PenaltyRate,Accountnumber,Subproductcode,GL_loans,Restructured) VALUES "
                    sql += "('" + txtmember.SelectedValue + "',"
                    sql += "'" + txtlno.Text + "',"
                    sql += "'" + txtcoll.SelectedValue.ToString + "',"
                    sql += "'" + txtfunding.SelectedValue.ToString + "',"
                    sql += "" + Double.Parse(txtlnamnt.Text).ToString + ","
                    sql += "" + Double.Parse(txtdeduction.Text).ToString + ","
                ...
                Else
                    ...
                    If Not rd.Read Then
                        If MessageBox.Show("Are you sure you want to save this loan?", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                            gen_pnnumber()
                            conn()
                            sql = "INSERT INTO Loans(Memcode,Pnnumber,CollCode,FundCode,LoanAmnt,TotalDeduction,netproceeds,IntRate,modeofpayment,LoanDate,Status,Released,userid,loantype,cycle,firstpaymentdate,releasedate,ctrcode,grpcode,col_type,Purpose,PayNo,PenaltyRate,Accountnumber,Subproductcode,GL_loans) VALUES "
                            sql += "('" + txtmember.SelectedValue + "',"
                            sql += "'" + txtlno.Text + "',"
                            sql += "'" + txtcoll.SelectedValue.ToString + "',"
                            sql += "'" + txtfunding.SelectedValue.ToString + "',"
    ...
    Private Sub txtlnamnt_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtlnamnt.KeyPress
        Try
            ...
            If e.KeyChar = Chr(13) Then GetNextControl(txtloantype, True).Focus() 'Enter key moves to next control in Tab order
        Catch ex As Exception
    ...
    Private Sub dtgrid_items_CellValidated(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.CellValidatedEventArgs) Handles dtgrid_items.CellValidated
    End Sub
    ...
    Private Sub dtgrid_items_CellValidating(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.CellValidatingEventArgs) Handles dtgrid_items.CellValidating
        'Try
        '    If e.ColumnIndex = 1 Then
        '        conn()
        '        sql = "SELECT * from Iteminventory where reference ='" + dtgrid_items.CurrentRow.Cells(0).Value + "'"
    ...
    Private Sub dtgrid_items_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles dtgrid_items.Validated
        ...
        For i As Integer = 0 To dtgrid_items.Rows.Count - 1
            If dtgrid_items.Rows(i).Cells(3).Value <> "" Then
                ttlint = ttlint + Double.Parse(dtgrid_items.Rows(i).Cells(4).Value)
                ttlprn = ttlprn + Double.Parse(dtgrid_items.Rows(i).Cells(3).Value)
            End If
        Next
        Try
    ...
    Private Sub txtsubproduct_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtsubproduct.KeyDown
        If e.KeyCode = Keys.Enter Then
            cboterms.Focus()
        End If
    End Sub

    Private Sub txtsubproduct_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtsubproduct.Validated
        gen_interest()
    End Sub

    Private Sub gen_interest()
        conn()
        sql = "SELECT Decimal_rate,Code from SubProducts where code ='" + txtsubproduct.SelectedValue + "'"
        cmd = New SqlCommand(sql, myConn)
        myConn.Open()
        rd = cmd.ExecuteReader
        If rd.Read Then
            subproduct = rd("Code")
            subrate = rd("Decimal_rate")
        End If
        rd.Close()
    End Sub
    ...
    Private Sub dtgrid_items_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dtgrid_items.Click
